...
kokoro_migrate_merge_defaults() {
    ...
    jq -s '
      ...
      | .inbound.xhttp.mode = (if (.inbound.xhttp.mode // "") == "" then $d.inbound.xhttp.mode else .inbound.xhttp.mode end)
      | .inbound.xhttp.obfs = (.inbound.xhttp.obfs // $d.inbound.xhttp.obfs)
      | .inbound.xhttp.xmux = (.inbound.xhttp.xmux // $d.inbound.xhttp.xmux)
      | .inbound.xhttp.socket = (.inbound.xhttp.socket // $d.inbound.xhttp.socket)
      | .inbound.xhttp.socket_path = (if (.inbound.xhttp.socket_path // "") == "" then $d.inbound.xhttp.socket_path else .inbound.xhttp.socket_path end)
      | .fallback.type = (if (.fallback.type // "") == "" then $d.fallback.type else .fallback.type end)
      | .fallback.root = (if (.fallback.root // "") == "" then $d.fallback.root else .fallback.root end)
      | .fallback.proxy_url = (.fallback.proxy_url // $d.fallback.proxy_url)
