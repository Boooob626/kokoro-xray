...
- Edge single-node: VLESS XHTTP TLS by default, or REALITY when selected
...
During edge setup, press Enter for `tls` unless you need `reality`. HY2 is enabled by default as a separate UDP acceleration option; keep port `443` unless you need a custom UDP port, and set the HY2 SNI to your domain.
...
- TLS mode uses the official Caddy release binary with checksum verification.
