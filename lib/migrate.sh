...
kokoro_migrate_merge_defaults() {
    ...
    jq -s '
      ...
      | .paths.hy2_cert = (if (.paths.hy2_cert // "") == "" then $d.paths.hy2_cert else .paths.hy2_cert end)
      | .paths.hy2_key = (if (.paths.hy2_key // "") == "" then $d.paths.hy2_key else .paths.hy2_key end)
      | .inbound.hy2.enabled = (.inbound.hy2.enabled // $d.inbound.hy2.enabled)
